...
constexpr float kFullDetailZoom = 32.0f;
...
std::int32_t LODController::Level(float zoom) const {
    ...
    const auto clampedZoom = std::clamp(zoom, 1.0f, kFullDetailZoom);
    const auto detail = std::log2(clampedZoom) / std::log2(kFullDetailZoom);
